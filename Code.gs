...
function _readSheetFromSS(ss, sheetName) {
  ...
  // Trim header names to prevent whitespace mismatches when columns are renamed manually
...
function saveRow(sheetName,obj,sess,label) {
  ...
  upsertRow(sheetName,obj); audit('EDIT',label,'Saved '+label+': '+(obj.name||obj.id)); return {ok:true};
}
function deleteRecord(sheetName,id,sess,label) {
  requirePerm(sess,['manage_config']); deleteById(sheetName,id);
  audit('DELETE',label,'Deleted '+label+' id='+id); return {ok:true};
}
...
function audit(action,property,details) {
  // Use the no-flush writer — UUIDs are never pre-existing so skipping findRow scan is safe
  try { _auditNoFlush(action, property||'', details||''); } catch(_) {}
}
...
function handleLogin(payload) {
  ...
  try { _auditNoFlush('LOGIN',user.username,'@'+user.username+' signed in'); } catch(_) {}
...
function _auditNoFlush(action,property,details) {
  try {
    ...
    const row=headers.map(h=>{
      ...
      if(h==='ts') return new Date().toISOString();
      if(h==='action') return action||'';
      if(h==='property') return property||'';
      if(h==='user') return '';
...
function handleLogout(token) { if(token) _sessionDel(token); return {ok:true}; }
...
function handleResetPassword(payload) {
  try {
    ...
    SpreadsheetApp.flush();
    _deleteOTP(username);
    audit('EDIT',username,'@'+username+' reset password via OTP');
    return {ok:true};
...
function saveProperty(p,sess) {
  ...
  audit(isEdit?'EDIT':'ADD',p.name,`${isEdit?'Updated':'Registered'} ${p.doctype} — ${p.propno}`);
  return {ok:true,record:p};
}
function deleteProperty(id,sess) {
  requirePerm(sess,['delete_property']);
  const d1=deleteById(SH.PAR_RECORDS,id), d2=deleteById(SH.ICS_RECORDS,id), d3=deleteById(SH.PROPERTIES,id);
  if(!d1&&!d2&&!d3) return {ok:false,error:'Property not found.'};
  audit('DELETE','Property','Deleted property id='+id);
  return {ok:true};
}
...
function savePRS(payload, sess) {
  ...
  audit('PRS', payload.name,
    `PRS ${payload.prsNo}: ${payload.prsPurpose} — ${payload.propno} (${payload.doctype})`);
...
function saveRealProperty(r,sess) {
  ...
  audit(isEdit?'EDIT':'ADD',r.name,`${isEdit?'Updated':'Registered'} real property ${r.recno}`);
...
function saveHistory(entry,sess) {
  ...
  if(entry.type==='transfer'&&entry.propId) {
    ...
    if(prop) {
      ...
      audit('TRANSFER',prop.name,'Transferred to @'+entry.officer+' ('+entry.dept+')');
    }
  }
  return {ok:true};
}
...
function uploadPDF(payload,sess) {
  requirePerm(sess,['add_property','edit_property']);
  if(!payload.propId) return {ok:false,error:'propId required.'};
  ...
  const blob=Utilities.newBlob(bytes,'application/pdf',payload.fileName||'document.pdf');
  const file=folder.createFile(blob);
  file.setSharing(DriveApp.Access.ANYONE_WITH_LINK,DriveApp.Permission.VIEW);
  const link='https://drive.google.com/file/d/'+file.getId()+'/view';
  const {prop,sh}=_findProp(payload.propId);
  if(prop) { prop.pdfLink=link; prop.pdfName=payload.fileName||'document.pdf'; upsertRow(sh,prop); }
  return {ok:true,link,fileName:payload.fileName};
}
...
function saveUser(u,sess) {
  ...
  audit(isEdit?'EDIT':'ADD',u.name,(isEdit?'Updated':'Created')+' @'+u.username+' ('+u.role+')');
...
function deleteUser(id,sess) {
  ...
  audit('DELETE',target.name,'Deleted @'+target.username);
  return {ok:true};
}
...
function doChangePassword(payload,token) {
  try {
    ...
    sh.getRange(targetRow,pwdCol+1).setValue(newPwd);
    SpreadsheetApp.flush();
    audit('EDIT',username,'@'+username+' changed password');
    return {ok:true};
...
function bulkImport(payload,sess) {
  ...
  audit('IMPORT','Bulk Import','Imported '+imported+' '+(type||'property')+' record(s) by @'+sess.username);
